    ...
    if dataset_attr.load_from in ["script", "file"]:
        for i in range(len(images)):
            if isinstance(images[i], str):
                if isinstance(data_args.image_dir, str):
                    if os.path.isfile(os.path.join(data_args.image_dir, images[i])):
                        images[i] = os.path.join(data_args.image_dir, images[i])
                elif isinstance(data_args.image_dir, list):
                    for data_dir in data_args.image_dir:
                        if os.path.isfile(os.path.join(data_dir, images[i])):
