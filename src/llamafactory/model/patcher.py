...
def patch_processor(
    ...
    if getattr(config, "vision_config", None) is not None:  # visual models
        ...
        if getattr(model_args, "image_resolution", None) is not None:
            setattr(processor, "image_resolution", model_args.image_resolution)
            setattr(processor, "image_max_pixels", model_args.image_resolution)#NOTE 如果指定了旧参数，更新
        if getattr(model_args, "video_resolution", None) is not None:
            setattr(processor, "video_resolution", model_args.video_resolution)
            setattr(processor, "video_max_pixels", model_args.image_resolution)#NOTE 如果指定了旧参数，更新
